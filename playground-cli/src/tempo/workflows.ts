...
import { decodeFunctionResult, encodeFunctionData, parseAbi, toHex, zeroAddress, type Address } from "viem";
...
import {
  ...
  transferSummary, validatePrepared, validateSignature, validateSigned, writeArtifact,
  type BroadcastRecord, type PreparedTransaction,
} from "./artifacts.ts";
import {
  currencySchema, feeManagerAddress, formatSchema, getTempoConfiguration, hashSchema,
...
export async function prepareTempoTransfer(runtime: TempoRuntime, options: TempoOptions, dependencies: TempoDependencies = {}) {
  ...
  const feeAsset = format.data === "tempo" ? await rpc.metadata(parseAddress(
    await runtime.interaction.text("Fee token contract address (sender pays)", options.feeToken), "Fee token",
  )) : undefined;
  ...
  const transaction = format.data === "tempo" ? {
  ...
  const explicitGasLimit = integerOption(options.gasLimit, "Gas limit");
  const gasLimit = explicitGasLimit ?? ((await rpc.quantity("eth_estimateGas", [transaction])) * 120n + 99n) / 100n;
  const fields: Omit<PreparedTransaction, "unsigned_transaction"> = {
    ...
    fee_asset: feeAsset, fee_payer: "sender", nonce: Number(nonce), gas_limit: gasLimit.toString(),
  ...
  runtime.output.result({ status: "prepared", file: resolve(out), ...transferSummary(parsed.data), gas_limit_source: explicitGasLimit === undefined ? "RPC estimate plus 20 percent" : "explicit input" });
}
...
export async function signTempoTransfer(runtime: TempoRuntime, options: TempoOptions, dependencies: TempoDependencies = {}) {
  ...
  const hash = await validateSignature(prepared, result.signed_transaction);
  writeArtifact(out, {
    version: 1, kind: "tempo-signed", prepared, turnkey_type: transactionType.data,
    activity_id: result.activity_id, signed_transaction: result.signed_transaction, transaction_hash: hash,
  });
...
export async function broadcastTempoTransfer(runtime: TempoRuntime, options: TempoOptions, dependencies: TempoDependencies = {}) {
  ...
  const before = await rpc.snapshot([signed.prepared.source, signed.prepared.destination], await balanceTokens(rpc, signed.prepared));
