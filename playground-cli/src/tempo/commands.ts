...
export function registerTempoCommands(program: Command, contextFor: (command: Command) => Promise<TempoRuntime>): void {
  const tempo = program.command("tempo")
    .description("Inspect Tempo and run direct protocol experiments")
    .action(async (_options: unknown, command: Command) => runTempoMenu(await contextFor(command)));
  const cliOnly = tempo.command("cli-only")
    ...
    .addOption(new Option("--network <network>", "Explicit chain selection; also accepts TEMPO_NETWORK").choices(["mainnet", "moderato"]))
    .action(async (_options: unknown, command: Command) => runTempoMenu(await contextFor(command), command.optsWithGlobals<TempoOptions>()));
  ...
  register(assetOptions(cliOnly.command("inspect").description("Read chain ID and TIP-20 metadata")), inspectTempo);
  register(assetOptions(cliOnly.command("balances").description("Read token balances at one block"))
    .option("--address <address>", "Wallet address to inspect"), readTempoBalances);
  register(assetOptions(cliOnly.command("prepare").description("Prepare an unsigned Moderato token transfer; no signing or broadcast"))
    ...
    .option("--fee-token <address>", "Required for native Tempo; the source pays the fee")
  ...
  register(cliOnly.command("sign").description("Ask Turnkey to sign a prepared transaction, or resume its activity; no broadcast")
  ...
  register(cliOnly.command("broadcast").description("Submit a signed Moderato transaction once; an existing record resumes observation")
  ...
  register(cliOnly.command("receipt").description("Read receipt, actual token movements, fee evidence, and block balances")
...
export async function runTempoMenu(runtime: TempoRuntime, options: TempoOptions = {}): Promise<void> {
  if (!runtime.interaction.interactive) throw new UsageError("Choose a tempo cli-only subcommand in non-interactive mode");
  ...
  while (true) {
    ...
    try {
      ...
      else if (error instanceof PlaygroundError) runtime.output.error(error.message);
      else throw error;
    }
  }
}
